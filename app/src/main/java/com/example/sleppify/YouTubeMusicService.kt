...
class YouTubeMusicService @JvmOverloads constructor(
    ...
    // ----- Replacement Candidates for Restricted Tracks -----
    ...
    /**
     * Searches for replacement candidates for a restricted track.
     * Filters out the original video, restricted IDs, non-embeddable, and short tracks.
     * Returns up to [maxCandidates] valid results on the main thread.
     */
    fun searchReplacementCandidates(
        ...
        val maxResults = maxOf(5, maxCandidates * 3) // Fetch more to compensate for filtering

        executor.execute {
            try {
                val pageResult = performSearchRequest(normalized, maxResults, apiKey, null)
                val restrictedIds = OfflineRestrictionStore.getRestrictedIds(appContext)
                val originalId = originalVideoId.trim()

                // Filter out original + restricted
                ...
                for (item in pageResult.tracks) {
                    ...
                    if (OfflineRestrictionStore.isRestricted(restrictedIds, item.videoId)) continue
                    eligible.add(item)
                ...
                // Fetch duration & embeddable info via public API key
