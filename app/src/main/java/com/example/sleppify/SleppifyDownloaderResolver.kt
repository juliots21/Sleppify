...
object SleppifyDownloaderResolver {
    ...
    fun downloadViaProxy(
        ...
        return try {
            connection = (URL(endpoint).openConnection() as HttpURLConnection).apply {
                ...
                if (!youtubeCookie.isNullOrBlank()) {
                    setRequestProperty("X-Youtube-Cookie", youtubeCookie)
                    Log.d(TAG, "audio: sending X-Youtube-Cookie len=${youtubeCookie.length} to $endpoint")
                } else {
                    Log.w(TAG, "audio: NO youtube cookie for $endpoint")
                }
                if (isResume) {
            ...
            if (!resuming && !freshStart) {
                ...
                Log.w(TAG, "proxy_fail id=$videoId $serverLabel http=$code elapsed=${System.currentTimeMillis() - startMs}ms err=$errBody")
                return false
            }
     ...
     * Same retry/resume logic as [downloadViaProxy] but hits /api/video endpoint.
     */
    fun downloadVideoViaProxy(
        ...
        return try {
            connection = (URL(endpoint).openConnection() as HttpURLConnection).apply {
                ...
                if (!youtubeCookie.isNullOrBlank()) {
                    setRequestProperty("X-Youtube-Cookie", youtubeCookie)
                    Log.d(TAG, "video: sending X-Youtube-Cookie len=${youtubeCookie.length} to $endpoint")
                } else {
                    Log.w(TAG, "video: NO youtube cookie for $endpoint")
                }
                if (isResume) {
                    setRequestProperty("Range", "bytes=$existingBytes-")
                }
